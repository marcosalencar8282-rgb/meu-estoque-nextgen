...
# Cadastro dos endereços físicos do armazém (Estrutura de posições)
...
# Saldo de estoque mapeado por endereço (Onde o produto está e quanto tem lá)
...
# Histórico de movimentações de posições
...
if 'user' not in st.session_state:
    ...
    # 3. Entrada e Armazenagem Direcionada
        ...
        if not validar_perfil(['Almoxarife', 'Conferente']):
            st.error("Perfil sem autorização para entrada de estoque."); st.stop()
            ...
            with st.form("armazenagem"):
                ...
                if st.form_submit_button("Confirmar Armazenagem"):
                    if cod_prod and desc_prod:
                        # Insere o produto no endereço mapeado
                        ...
                        # Atualiza o status do endereço físico para Ocupado
                        st.session_state.locations.loc[st.session_state.locations['Endereço'] == endereco_destino, 'Status'] = 'Ocupado'
                        
                        # Histórico
                        novo_mov = pd.DataFrame([{
                            'Tipo': 'Entrada', 'Endereço Origem': 'Docas', 'Endereço Destino': endereco_destino,
                            'Produto': cod_prod, 'Quantidade': qtd, 'Data Hora': datetime.now().strftime('%d/%m/%Y %H:%M'), 'Operador': st.session_state.user
                        }])
                        st.session_state.movements = pd.concat([st.session_state.movements, novo_mov], ignore_index=True)
                        
                        registrar_auditoria(f"Armazenou prod {cod_prod} no endereço {endereco_destino}", "Inventário")
                        st.success(f"Produto armazenado com sucesso no endereço {endereco_destino}!")
                        st.rerun()
                    else:
                        st.warning("Preencha as informações do produto.")

    # 4. Movimentação Interna (Transferência de Palete/Caixa)
    elif opcao_menu == "Movimentação Interna":
        st.title("🔄 Transferência entre Endereços (Revezamento/Paletização)")
